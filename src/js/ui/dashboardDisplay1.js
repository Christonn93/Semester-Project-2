...
export async function displayProfileInformation() {
  ...
  if (storageToken) {
    ...
    if (user) {
      ...
      const displayUserImage = imageFactory(['ag-profile-img', 'shadow', 'ag-relative', 'mb-3'], 'userImage', `${imageUrl}`, 'Profile image');
      ...
      profileImage.append(displayUserImage, displayModalButton);

      const profileName = document.getElementById('profileName');
      profileName.append(displayUserName);

      const profileDetails = document.getElementById('profileDetails');
      profileDetails.append(displayUserEmail, displayUserCredits, displayUserListingAmount, displayWinAmount);

      const createNew = document.getElementById('profileDetails');
      createNew.append(sidebarDivider, displayLinkNewEntry);
    }
  }
}
